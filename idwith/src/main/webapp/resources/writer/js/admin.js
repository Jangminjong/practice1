...
function inputGroupAdd(){
	...
	var buttonClone = $(".mb-3 button:last-child").clone().css({'display' : ''}).prop('id',imgInput+imgInt);
	buttonClone.on('click',{inputid : imgInput+imgInt}, function(event){
...
function execDaumPostcode() {
    new daum.Postcode({
        oncomplete: function(data) {
			...
			// 자동으로 넣어준 필드의 라벨을 위로 옮김.
			document.getElementById('postlabel').style.transform = 'translateY(-120%)';
			document.getElementById('postlabel').style.fontSize = '12px';
			document.getElementById('addrlabel').style.transform = 'translateY(-120%)';
			document.getElementById('addrlabel').style.fontSize = '12px';
			document.getElementById('extaddrlabel').style.transform = 'translateY(-120%)';
			document.getElementById('extaddrlabel').style.fontSize = '12px';
            // 커서를 상세주소 필드로 이동한다.
